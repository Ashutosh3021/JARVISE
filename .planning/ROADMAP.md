...
## Phase B-1: Core Chat Foundation
**Goal:** Fix all bugs that prevent basic text chat from working end-to-end
**Depends on:** Phase 11
**Requirements:** BL-05, BL-06, ST-09, HW-04
**Success Criteria** (what must be TRUE):
  1. Exactly ONE system message sent to Ollama (no empty responses)
  2. Streaming yields clean text without Thought:/Action: artifacts
  3. Both basic and heavy tools exist in same registry
  4. Heavy tools lazy-initialized (no startup crashes)
  5. Router uses enum comparison (no string .value comparisons)
  6. Backend shares one agent/memory/router via app.state
  7. No route-level singletons in backend
  8. Dependencies install without kokoro package conflict
**Plans:** 4/4 plans complete

Plans:
- [x] B-1-01-PLAN.md — LLM Response Fixes (prompt_builder + agent)
- [x] B-1-02-PLAN.md — Tool Registry Fixes (brain/tools.py)
- [x] B-1-03-PLAN.md — Logging + Router + Backend + Dependencies

## Phase B-2: Voice Pipeline
**Goal:** Fix all bugs that prevent voice input → agent → voice output from working
**Depends on:** Phase B-1
**Requirements:** B-008, B-037, B-007, B-009, B-040, B-002
**Success Criteria** (what must be TRUE):
  1. TTS engine initializes without TypeError from KPipeline
  2. TTS speed adjustment uses librosa.resample, not np.interp
  3. VAD receives properly framed 20ms int16 PCM chunks
  4. STT audio normalization runs exactly once per transcription
  5. STT loads on correct device on first attempt (no double load)
  6. Voice transcription callback wired to router/agent before start()
**Plans:** 3/3 plans complete

Plans:
- [x] B-2-01-PLAN.md — TTS Fixes (KPipeline constructor + librosa speed)
- [x] B-2-02-PLAN.md — STT + VAD Fixes (double norm, VAD framing, CUDA detect)
- [x] B-2-03-PLAN.md — Voice Wiring (transcription callback → agent/router)

---

## Phase B-3: Web + Memory
**Goal:** Fix all bugs that prevent web search and memory from working correctly
**Depends on:** Phase B-1, B-2
**Requirements:** BUG-006, BUG-010, BUG-042, BUG-043, BUG-020, BUG-021
**Success Criteria** (what must be TRUE):
  1. Web search returns real results (DuckDuckGo selectors or duckduckgo-search package)
  2. Browser tool doesn't crash in async context (Playwright sync vs async)
  3. Browser validates URL before navigation (no empty URL crashes)
  4. Memory is injected into agent prompts correctly
  5. Chain templates have separate tool/action fields
  6. {previous_output} substitution works in chains
**Plans:** 3/3 plans complete

Plans:
- [ ] B-3-01-PLAN.md — Web Search Fix (duckduckgo-search + BrowserTool cleanup + URL validation)
- [ ] B-3-02-PLAN.md — Memory Integration (wire MemoryManager in main.py loop)
- [ ] B-3-03-PLAN.md — Chain Fixes (separate tool/action + output substitution)

---

## Phase B-4: Security Hardening
**Goal:** Close all security holes before any deployment
**Depends on:** Phase B-1, B-2, B-3
**Requirements:** BUG-022, BUG-023, BUG-024
**Success Criteria** (what must be TRUE):
  1. Auth enforcement on ALL routes including WebSocket and memory
  2. Code execution disabled by default, requires ENABLE_CODE_EXEC=true + user confirm
  3. Data files and creds removed from git tracking
**Plans:** 3/3 plans complete

---

## Phase B-5: Polish + Routing
**Goal:** Fix remaining issues and hardening
**Depends on:** Phase B-4
**Requirements:** BUG-029, BUG-030, BUG-034, BUG-035, BUG-036, BUG-039
**Success Criteria** (what must be TRUE):
...
| B-1. Core Chat Foundation | 4/4 | Complete    | 2026-03-21 |
